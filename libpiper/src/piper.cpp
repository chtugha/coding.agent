...
int piper_synthesize_start(struct piper_synthesizer *synth, const char *text,
    ...
    while (text_ptr != nullptr) {
        ...
        // Since we don't have terminator info from standard espeak_TextToPhonemes,
        // we'll just add basic sentence separation based on text content
        if (text_ptr == nullptr) {
            // End of text - add sentence break
