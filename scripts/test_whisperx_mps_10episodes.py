#!/usr/bin/env python3
"""
Test WhisperX with MPS backend on first and last 5 minutes of 10 cleaned episodes.
...
OUTPUT_DIR = "/Volumes/eHDD/moshi-rag-data/datasets/whisperx_mps_test"
...
def test_episode(episode_num):
    ...
    # Transcribe with WhisperX MPS
    start_time = time.time()
    whisperx_result = model.transcribe(first_5min_audio, batch_size=16)
    transcribe_time = time.time() - start_time
    
    whisperx_text = ' '.join([seg['text'].strip() for seg in whisperx_result['segments']])
    similarity = calculate_similarity(first_5min_original, whisperx_text)
    ...
    print(f"    WhisperX length: {len(whisperx_text)} chars", flush=True)
    ...
    # Transcribe with WhisperX MPS
    start_time = time.time()
    whisperx_result = model.transcribe(last_5min_audio, batch_size=16)
    transcribe_time = time.time() - start_time
    
    whisperx_text = ' '.join([seg['text'].strip() for seg in whisperx_result['segments']])
    similarity = calculate_similarity(last_5min_original, whisperx_text)
...
def main():
    """Main entry point."""
    print("="*70, flush=True)
    print("WhisperX MPS Test on 10 Episodes (First & Last 5 Minutes)", flush=True)
    print("="*70, flush=True)
    ...
    # Load WhisperX model with CPU (MPS not supported by faster-whisper/ctranslate2)
    # Note: WhisperX uses faster-whisper which doesn't support MPS
    # For MPS support, would need to use native PyTorch Whisper
    print("Loading WhisperX model with CPU backend...", flush=True)
    print("Note: MPS not supported by faster-whisper backend", flush=True)
    device = "cpu"
    compute_type = "int8"
    ...
    if all_results:
        ...
        print(f"Estimated speedup: {300/avg_time:.1f}x", flush=True)
        ...
        summary = {
            ...
            'estimated_speedup': 300/avg_time,
            'results': all_results
        }
