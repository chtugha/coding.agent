#!/usr/bin/env python3
"""
Test fixed waveform alignment on Episode 152 (has ads in middle).
"""
...
def main():
    # Paths for episode 152
    base_dir = "/Volumes/eHDD/moshi-rag-data/datasets/Gemischtes.Hack.Podcast"
    
    # Find episode 152 files
    import glob
    audio_files = glob.glob(os.path.join(base_dir, "*152*.mp3"))
    transcript_files = glob.glob(os.path.join(base_dir, "Gemischtes.Hack.Podcast.Transcript/transcripts/*152*.json"))
    
    if not audio_files:
        print("ERROR: Could not find episode 152 audio file")
        return
    
    if not transcript_files:
        print("ERROR: Could not find episode 152 transcript file")
        return
    
    audio_path = audio_files[0]
    transcript_path = transcript_files[0]
    
    print("=== Testing Fixed Waveform Alignment on Episode 152 ===\n")
    ...
    output_audio = os.path.join(output_dir, "episode_152_cleaned_fixed.wav")
    sf.write(output_audio, cleaned_audio, sr)
    print(f"Saved cleaned audio: {output_audio}")
    
    output_regions = os.path.join(output_dir, "kept_regions_ep152.json")
