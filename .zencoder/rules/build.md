...
- `DNEUTTS_COREML=ON` — enables CoreML for NeuTTS NeuCodec decoder (default ON)
...
Note: PyTorch/libtorch is **not** required at build or runtime. Kokoro TTS uses a pure C++ HAR source implementation (ktensor.h + har_source.h) with CoreML for inference. NeuTTS service uses llama.cpp for the backbone and CoreML for the NeuCodec decoder.
...
| NeuCodec decoder | `neutts-nano-german/neucodec_decoder.mlmodelc/` | Converted from ONNX (neuphonic/neucodec-onnx-decoder) |
