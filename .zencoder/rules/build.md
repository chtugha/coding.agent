...
- **ONNX Runtime**: expected at `third_party/onnxruntime/` (downloaded by `runmetoinstalleverythingfirst`, v1.24.3 arm64). The dylib is also copied to `bin/` so it is found at `@executable_path` at runtime.

Note: PyTorch/libtorch is **not** required at build or runtime. Kokoro TTS uses a pure C++ HAR source implementation (ktensor.h + har_source.h) with CoreML for inference. NeuTTS service uses llama.cpp for the backbone and **ONNX Runtime** (with CoreML EP) for the NeuCodec decoder — no conversion step required, the `.onnx` file is loaded directly at inference time.
...
| NeuTTS backbone | `neutts-nano-german/neutts-nano-german-Q8_0.gguf` | HuggingFace neuphonic/neutts-nano-german-q8-gguf |
| NeuCodec decoder | `neutts-nano-german/neucodec_decoder.onnx` | Downloaded from HuggingFace (neuphonic/neucodec-onnx-decoder), run via ONNX Runtime |
| NeuTTS reference | `neutts-nano-german/ref_codes.bin`, `ref_text.txt` | Encoded from greta.wav reference audio |
