...
## Internal Function
- **Inference**: Uses `whisper.cpp` optimized for Apple Silicon via **CoreML/Metal**, providing near-real-time transcription. Receives complete speech segments (typically 1-4 seconds) — no VAD logic is performed here.
- **Decoding**: GREEDY strategy (not beam search). On short 2-4s segments, greedy is ~3-5x faster than beam_size=5 with negligible accuracy difference. Temperature fallback handles errors.
