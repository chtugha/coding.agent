...
- **Micro-pause Detection**: Triggers speech-end after 400ms of silence (8 frames × 50ms), capturing word-boundary pauses instead of waiting for full sentence gaps. This keeps chunks at 1.5-4 seconds for fast Whisper inference.
- **Chunk Constraints**: Maximum 4s chunks (Whisper attention is O(n²)), minimum 0.5s (reject noise bursts). RMS energy filter (threshold 0.005) prevents near-silence hallucinations.
...
- **Context Preservation**: Includes 4 frames of pre-speech context audio for natural chunk boundaries.
