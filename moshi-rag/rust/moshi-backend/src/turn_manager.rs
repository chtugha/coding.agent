...
impl TurnManager {
    ...
    fn update_active_speaker(&mut self) -> bool {
        ...
        // vad values are speech probabilities: 0.0 = silence, 1.0 = user speaking.
        ...
        if past_warmup && vad_pos && self.active_speaker == TextRole::Model {
            // Switch to User right away if average speech probability exceeds threshold for VAD_WINDOW_SIZE steps.
            ...
            // Switch to Model if
            // 1. speech probability is below threshold (user is silent) for consecutive VAD_WINDOW_SIZE steps, and
